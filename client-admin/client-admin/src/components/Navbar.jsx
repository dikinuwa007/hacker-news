...
const [page, setPage] = useState("");
  const sendDataToParent = (data) => {
    let goToPage = data;
    props.onPageFromNavbar(goToPage);
  }
  const navigate = useNavigate()
  const logout = async () => {
    localStorage.removeItem('access_token')
    navigate('/login')
    Swal.fire('Logout Success','Anda berhasil Logout','success')
  ...
  const navList = (
    <ul className="mt-2 mb-4 flex flex-col gap-2 lg:mb-0 lg:mt-0 lg:flex-row lg:items-center lg:gap-6">
      ...
      >
        ...
        {/* <a href="#" className="flex items-center" onClick={()=>sendDataToParent("categories")}>
          Categories
        </a> */}
